...
class MapDownloader:
    ...
    def get_map(self,map_type, save=False, to_rgb=True):
        ...
        if to_rgb:
            region_map = np.array(Image.open(BytesIO(region_map_request)).convert('RGB')) / 255.0

        else:
            region_map = np.array(Image.open(BytesIO(region_map_request))) / 255.0
